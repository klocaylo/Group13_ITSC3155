...
def new_note():
    ...
    if session.get('user'):
        ...
        if request.method == 'POST':
            ...
            new_record = Note(title, text, today, color, favorite, session['user_id'])
...
def get_favorite():
    ...
    if session.get('user'):
        ...
        return render_template('favorite.html', notes=my_notes, user=session['user'])
    else:
        return redirect(url_for('login'))

@app.route('/notes/favorite/<note_id>', methods=['GET', 'POST'])
   ...
   #check if a user is saved in session
    if session.get('user'):
        ...
        if request.method == 'POST':
            ...
            return render_template('notes.html', note = my_note, user=session['user'])
    else:
        # user is not in session redirect to login
        return redirect(url_for('login'))

@app.route('/notes/unfavorite/<note_id>', methods=['GET', 'POST'])
