...
# class that maos to the user and note tables in our database
class Note(db.Model):
...
# class that maps to the user and todo tables in our database
class Todo(db.Model):
    id = db.Column("id", db.Integer, primary_key=True)
    title = db.Column("title", db.String(200))
    date = db.Column("date", db.String(50))
    checks = db.Column("checks", db.Boolean(False))

    # can create a foreign key; referencing the id variable in the User class,
    # so that is why it is lowercase u
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)

    def __init__(self, title, date, checks, user_id):
        self.title = title
        self.date = date
        self.checks = checks
        self.user_id = user_id
